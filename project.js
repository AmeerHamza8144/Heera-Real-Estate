...
function renderProjectPlans(container, project) {
  ...
  if (structuredPlans.length) {
    const headers = ['Plan', 'Booking', 'Monthly instalment', 'Half-yearly count', 'Half-yearly instalment', 'On possession', 'Balloting', 'Other Payment', 'Total price'];
    ...
    planGroups.forEach((group) => {
      const rows = group.plans.map((plan) => {
        const values = [plan.size_label, plan.booking_amount, plan.monthly_installment, plan.half_yearly_count, plan.half_yearly_installment, plan.on_possession, plan.balloting, plan.other_payment, plan.total_price];
        return `<tr>${values.map((value, index) => `<td data-label="${escapeHtml(headers[index])}">${escapeHtml(value || '—')}</td>`).join('')}</tr>`;
      }).join('');
